...
function InsightCard({ insight }: { insight: Insight }) {
  const c = COLOR_MAP[insight.type], Icon = insight.icon;
  return (
    <div className={`rounded-2xl p-4 border ${c.bg}`}>
      <div className="flex items-start gap-3">
        <Icon size={18} className={`${c.icon} mt-0.5 flex-shrink-0`} />
        <div><p className={`text-sm font-bold ${c.title} mb-1`}>{insight.title}</p><p className="text-xs text-muted-foreground leading-relaxed">{insight.desc}</p></div>
      </div>
    </div>
  );
}
    ...
    <div className="glass-card rounded-3xl p-6 border border-white/[0.06] flex flex-col items-center">
      ...
      <div className="relative w-36 h-36">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
          ...
          <circle cx="60" cy="60" r={r} fill="none" stroke={color} strokeWidth="10" strokeDasharray={circ} strokeDashoffset={offset} strokeLinecap="round" style={{ transition: "stroke-dashoffset 1.2s ease" }} />
        </svg>
...
export default function AICoach() {
  ...
  const stats = useMemo(() => {
    ...
    for (const t of closed) { const em = t.emotion || "neutral"; if (!emotionPnl[em]) emotionPnl[em] = []; emotionPnl[em].push(t.pnl ?? 0); }
    const em2avg = (e: Record<string, number[]>) => Object.entries(e).map(([k, v]) => ({ em: k, avg: v.reduce((s, p) => s + p, 0) / v.length }));
  ...
  const insights = useMemo((): Insight[] => {
    ...
    const r: Insight[] = [];
    if (stats.winRate >= 60) r.push({ type: "success", icon: CheckCircle, title: "High Win Rate", desc: `${stats.winRate.toFixed(1)}% puts you in the top tier. Focus on maintaining consistency.` });
    else if (stats.winRate < 45) r.push({ type: "danger", icon: XCircle, title: "Win Rate Below 45%", desc: "Review your entry criteria carefully. Consider paper trading new setups." });
    if (stats.rr >= 2) r.push({ type: "success", icon: Shield, title: "Excellent Risk/Reward", desc: `${stats.rr.toFixed(2)}:1 R/R is professional grade. You're extracting maximum value.` });
    ...
    if (stats.best && stats.best.avg > 0) r.push({ type: "tip", icon: Lightbulb, title: `Peak State: "${stats.best.em}"`, desc: `You perform best when ${stats.best.em}. Identify what creates this mindset and replicate it.` });
    if (stats.winStreak >= 3) r.push({ type: "warning", icon: AlertTriangle, title: "Hot Streak — Stay Grounded", desc: `${stats.winStreak} wins in a row. Don't let overconfidence lead to oversizing.` });
    if (stats.mistakeTrades.length > 3) r.push({ type: "warning", icon: Target, title: "Recurring Mistakes", desc: `${stats.mistakeTrades.length} trades have logged mistakes. Review them for repeating patterns.` });
    ...
    if (r.length < 2) r.push({ type: "tip", icon: Lightbulb, title: "Keep Logging", desc: "More data = better insights. Log every trade including emotions and notes." });
    return r;
  }, [stats, closed.length]);
  ...
  return (
    <div className="flex flex-col">
      <header className="page-header px-5 py-3.5">
        <div className="flex items-center gap-2"><Brain size={17} className="text-primary" /><h1 className="text-lg font-bold font-serif text-foreground">AI Coach</h1></div>
      ...
      <div className="px-4 pt-4 pb-6 flex flex-col gap-4">
        ...
        <div className="glass-card rounded-3xl p-5 border border-white/[0.06]">
          ...
          <div className="flex flex-col gap-4">
            {bars.map(({ label, value }) => {
              ...
              return (
                <div key={label}>
                  <div className="flex justify-between mb-1.5"><span className="text-xs text-muted-foreground">{label}</span><span className="text-xs font-bold text-foreground">{value.toFixed(0)}%</span></div>
                  <div className="h-2 bg-muted rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-1000" style={{ width: `${value}%`, background: c }} /></div>
                </div>
              );
            })}
          </div>
        ...
        <div className="glass-card rounded-3xl p-5 border border-white/[0.06]">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            {[
              ...
              { label: "Avg Win",     value: format(stats.avgWin),      color: "text-emerald-400" },
              { label: "Avg Loss",    value: format(stats.avgLoss),     color: "text-red-400" },
        ...
        <div>
          ...
          <div className="flex flex-col gap-3 stagger">{insights.map((ins, i) => <InsightCard key={i} insight={ins} />)}</div>
        </div>
      </div>
    </div>
  );
