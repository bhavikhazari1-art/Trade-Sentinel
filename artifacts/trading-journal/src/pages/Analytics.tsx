...
const ttStyle = {
  contentStyle: {
    background: "rgba(10,13,22,.95)", border: "1px solid rgba(255,255,255,.08)",
    borderRadius: 14, fontSize: 11, backdropFilter: "blur(20px)",
  },
};
...
export default function Analytics() {
  ...
  const bySymbol = useMemo(() => {
    ...
    return Object.entries(map)
      ...
      .sort((a, b) => b.pnl - a.pnl).slice(0, 7);
  }, [closed]);
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="glass-card rounded-2xl p-4 border border-white/[0.06]">
                <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1.5">Total P&L</p>
                <p className={`text-2xl font-bold font-serif ${getPnlColor(totalPnl)}`}>{formatCurrency(totalPnl)}</p>
              </div>
              <div className="glass-card rounded-2xl p-4 border border-white/[0.06]">
                <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1.5">Win Rate</p>
                <p className={`text-2xl font-bold font-serif ${winRate >= 50 ? "text-emerald-400" : "text-red-400"}`}>
                  {winRate.toFixed(1)}%
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-6">
                <div className="flex-shrink-0">
                  <ResponsiveContainer width={110} height={110}>
                    <PieChart>
                      <Pie data={winLossPie} cx="50%" cy="50%" innerRadius={32} outerRadius={52}
                        dataKey="value" strokeWidth={0}>
                        {winLossPie.map((e, i) => <Cell key={i} fill={e.color} />)}
                      </Pie>
                    </PieChart>
                  </ResponsiveContainer>
                </div>
                  ...
                  <AreaChart data={equityCurve} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
                    <defs>
                      <linearGradient id="eqGrad" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%"  stopColor={totalPnl >= 0 ? GREEN : RED} stopOpacity={0.3} />
                        <stop offset="95%" stopColor={totalPnl >= 0 ? GREEN : RED} stopOpacity={0} />
                      </linearGradient>
                    </defs>
                    <XAxis dataKey="date" hide />
                    <YAxis hide />
                    <Tooltip {...ttStyle} formatter={(v: number) => [formatCurrency(v), "Equity"]} />
                    <Area type="monotone" dataKey="pnl"
                      stroke={totalPnl >= 0 ? GREEN : RED} strokeWidth={2}
              ...
              <Section icon={TrendingUp} title="Monthly P&L">
                <ResponsiveContainer width="100%" height={130}>
                  <BarChart data={monthlyPnl} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
                    ...
                    <YAxis hide />
                    <Tooltip {...ttStyle} formatter={(v: number) => [formatCurrency(v), "P&L"]} />
                    <Bar dataKey="pnl" radius={[6, 6, 0, 0]}>
                      {monthlyPnl.map((e, i) => <Cell key={i} fill={e.pnl >= 0 ? GREEN : RED} fillOpacity={0.85} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </Section>
              ...
              <Section icon={BarChart3} title="P&L by Symbol">
                <ResponsiveContainer width="100%" height={Math.max(120, bySymbol.length * 30)}>
                  <BarChart data={bySymbol} layout="vertical" margin={{ top: 0, right: 8, left: 4, bottom: 0 }}>
                    ...
                    <YAxis type="category" dataKey="sym" tick={{ fontSize: 11, fill: "#94a3b8" }} axisLine={false} tickLine={false} width={42} />
                    <Tooltip {...ttStyle} formatter={(v: number) => [formatCurrency(v), "P&L"]} />
                    <Bar dataKey="pnl" radius={[0, 6, 6, 0]}>
                      {bySymbol.map((e, i) => <Cell key={i} fill={e.pnl >= 0 ? GREEN : RED} fillOpacity={0.85} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </Section>
                ...
                <div className="flex flex-col divide-y divide-border/30">
                  {byEmotion.map(em => (
                    <div key={em.em} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                      <div>
                        <p className="text-sm text-foreground font-semibold capitalize">{em.em}</p>
                        <p className="text-[11px] text-muted-foreground">{em.count} trades · {em.wr}% WR</p>
                      </div>
                ...
                <div className="flex flex-col divide-y divide-border/30">
                  {bySetup.map(s => (
                    <div key={s.setup} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                      <div>
                        <p className="text-sm text-foreground font-semibold">{s.setup}</p>
                        <p className="text-[11px] text-muted-foreground">{s.count} trades · {s.wr.toFixed(0)}% WR</p>
                      </div>
