...
import {
  ...
  Flame, Shield, Zap, Award, BarChart2, Clock, Plus, AlertTriangle,
} from "lucide-react";
    ...
    <div className="glass-card rounded-2xl p-4 flex flex-col gap-2 card-hover border border-white/[0.06]">
      <div className="flex items-center justify-between">
        ...
        <div className={`p-1.5 rounded-xl ${color ?? "bg-primary/10"}`}>
          <Icon size={13} className={trend === "up" ? "text-emerald-400" : trend === "down" ? "text-red-400" : "text-primary"} />
        </div>
      </div>
...
const ttStyle = {
  contentStyle: { background: "rgba(10,13,22,.95)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 14, fontSize: 11, backdropFilter: "blur(20px)" },
};

function StreakHeatmap({ dailyPnl, currentStreak }: { dailyPnl: Record<string, number>; currentStreak: number }) {
  const days = useMemo(() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      result.push({ key, label: d.toLocaleDateString("en-US", { month: "short", day: "numeric" }), pnl: dailyPnl[key] ?? null, isToday: i === 0 });
    }
    return result;
  }, [dailyPnl]);

  const profitDays = days.filter(d => (d.pnl ?? -1) > 0).length;
    ...
    <div className="glass-card rounded-3xl p-4 border border-white/[0.06]">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className={cn("p-1.5 rounded-xl", currentStreak >= 1 ? "bg-primary/10" : "bg-muted")}>
            <Flame size={14} className={currentStreak >= 3 ? "text-primary" : "text-muted-foreground"} />
          </div>
          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">30-Day Streak</span>
        </div>
        {currentStreak > 0 ? (
          <div className={cn("flex items-center gap-1.5 rounded-full px-3 py-1 border", currentStreak >= 5 ? "bg-primary/20 border-primary/40 pulse-gold" : "bg-primary/10 border-primary/25")}>
          ...
          return (
            <div key={day.key}
              ...
              className={cn(
                ...
                isProfit ? (isBig ? "bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]" : "bg-emerald-400/70") :
                isLoss   ? (isBig ? "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]"   : "bg-red-500/60")   :
                "bg-white/[0.05]",
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-border/30">
        <div className="flex items-center gap-3">
          {[["bg-emerald-400/70", `${profitDays} profit`], ["bg-red-500/60", `${lossDays} loss`], ["bg-white/[0.05]", `${30 - activeDays} off`]].map(([bg, label]) => (
        ...
        {activeDays > 0 && (
          <span className={cn("text-[10px] font-bold", profitDays > lossDays ? "text-emerald-400" : "text-muted-foreground")}>
...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    let streak = 0; for (const t of sorted) { if ((t.pnl ?? 0) > 0) streak++; else break; }
    let maxStreak = 0, cur = 0;
    for (const t of [...closed].reverse()) { if ((t.pnl ?? 0) > 0) { cur++; maxStreak = Math.max(maxStreak, cur); } else cur = 0; }
  ...
  const dailyPnl = useMemo(() => {
    ...
    for (const t of trades) { if (t.pnl != null) map[t.entryDate] = (map[t.entryDate] ?? 0) + t.pnl; }
    return map;
  }, [trades]);

  const dayStreak = useMemo(() => {
    const sorted = Object.entries(dailyPnl).sort(([a], [b]) => b.localeCompare(a));
    let s = 0; for (const [, p] of sorted) { if (p > 0) s++; else break; }
    return s;
  }, [dailyPnl]);
  ...
  return (
    <div className="flex flex-col">
      <header className="page-header px-5 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {displayStreak > 0 && (
            <div className={cn("flex items-center gap-1 rounded-full px-2.5 py-1 border", displayStreak >= 5 ? "bg-primary/25 border-primary/40 pulse-gold" : "bg-primary/15 border-primary/25")}>
      ...
      <div className="px-4 pt-4 pb-6 flex flex-col gap-4">
        ...
        {error && (
          <div className="flex items-start gap-3 bg-red-500/10 border border-red-500/25 rounded-2xl p-4">
            ...
            <div>
              <p className="text-sm font-semibold text-red-300">Could not load trades</p>
              <p className="text-[11px] text-red-400/80 mt-0.5 leading-relaxed">{error}</p>
            </div>
          </div>
        )}
        ...
        <div className="glass-card rounded-3xl p-5 border border-white/[0.06] relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mt-3">
            ...
            {stats.open.length > 0 && <><span className="w-1 h-1 rounded-full bg-muted-foreground/40" /><span className="text-xs text-blue-400 font-medium">{stats.open.length} open</span></>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 stagger">
          ...
          <StatCard label="Avg Win"     value={format(stats.avgWin)}           icon={TrendingUp}   trend="up"   sub="per trade" />
          <StatCard label="Avg Loss"    value={format(stats.avgLoss)}          icon={TrendingDown} trend="down" sub="per trade" color="bg-red-500/10" />
        </div>
          ...
          <div className="glass-card rounded-2xl p-4 border border-white/[0.06]">
            ...
            <ResponsiveContainer width="100%" height={100}>
              <AreaChart data={equityCurve} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="pnl" stroke={stats.totalPnl >= 0 ? "#10b981" : "#ef4444"} strokeWidth={2} fill="url(#pnlGrad)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            { icon: Award,    label: "Level",        value: stats.winRate >= 60 ? "Elite" : stats.winRate >= 40 ? "Good" : "Grow",
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/add"><button className="flex items-center gap-1 text-xs text-primary font-semibold tap-small"><Plus size={13} /> Add</button></Link>
          </div>
          ...
          {trades.length === 0 ? (
            <div className="glass-card rounded-3xl p-8 border border-white/[0.06] text-center flex flex-col items-center gap-3">
              ...
              <div><p className="text-sm font-semibold text-foreground">No trades yet</p><p className="text-xs text-muted-foreground mt-1">Log your first trade to get started</p></div>
              <Link href="/add"><button className="bg-primary text-primary-foreground text-xs font-bold px-6 py-2.5 rounded-full hover:brightness-110 active:scale-95 transition-all">Log First Trade</button></Link>
            </div>
          ) : (
            <div className="flex flex-col gap-2 stagger">
              {trades.slice(0, 5).map(t => (
                <div key={t.id} className="glass-card rounded-2xl px-4 py-3.5 border border-white/[0.06] flex items-center justify-between card-hover">
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-xs font-bold flex-shrink-0 ${t.direction === "long" ? "bg-emerald-400/15 text-emerald-400" : "bg-red-400/15 text-red-400"}`}>
                      {t.direction === "long" ? "↑" : "↓"}
                    </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    {t.pnl != null ? (
                      <><div className={`text-sm font-bold ${getPnlColor(t.pnl)}`}>{format(t.pnl)}</div>
                      {t.pnlPercent != null && <div className={`text-[10px] font-medium ${getPnlColor(t.pnlPercent)}`}>{formatPercent(t.pnlPercent)}</div>}</>
