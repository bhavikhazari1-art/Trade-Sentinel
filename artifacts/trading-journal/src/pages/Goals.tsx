...
import { Target, Flame, Trophy, TrendingUp, Shield, Edit3, Check, X, Zap, Star, BarChart3, AlertTriangle } from "lucide-react";
    ...
    <div className={cn("glass-card rounded-3xl p-4 border transition-all card-hover", done ? "border-yellow-400/30 bg-yellow-400/5" : "border-white/[0.06]")}>
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-xl" style={{ background: `${color}20` }}><Icon size={13} style={{ color }} /></div>
          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{label}</span>
        </div>
        {done && <div className="flex items-center gap-1 bg-yellow-400/15 border border-yellow-400/25 rounded-full px-2 py-0.5"><Trophy size={9} className="text-yellow-400" /><span className="text-[9px] font-bold text-yellow-400">Done!</span></div>}
      </div>
      <div className="flex items-center gap-4">
        <div className="relative flex-shrink-0">
          <ProgressRing pct={pct} size={68} color={color} />
          <div className="absolute inset-0 flex items-center justify-center"><span className="text-xs font-bold text-foreground">{pct.toFixed(0)}%</span></div>
        </div>
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-1000" style={{ width: `${pct}%`, background: color }} /></div>
        </div>
      </div>
    </div>
  );
...
function EditSheet({ goals, onSave, onClose, format }: { goals: GoalsType; onSave: (g: GoalsType) => Promise<void>; onClose: () => void; format: (v: number) => string }) {
  const [form, setForm] = useState<GoalsType>({ ...goals });
  const [saving, setSaving] = useState(false);
  const set = (k: keyof GoalsType, v: string) => setForm(f => ({ ...f, [k]: parseFloat(v) || 0 }));

  const fields: { key: keyof GoalsType; label: string; prefix?: boolean; suffix?: string }[] = [
    { key: "weeklyPnl",       label: "Weekly P&L Target",  prefix: true },
    { key: "monthlyPnl",      label: "Monthly P&L Target", prefix: true },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-sm bg-card border border-border rounded-t-3xl shadow-2xl" style={{ paddingBottom: "max(env(safe-area-inset-bottom,0px), 16px)" }}>
        <div className="px-5 pt-4 pb-2 flex items-center justify-between">
          <h3 className="text-base font-bold font-serif text-foreground">Set Your Goals</h3>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center tap-small"><X size={15} className="text-muted-foreground" /></button>
        </div>
          ...
          {fields.map(({ key, label, prefix, suffix }) => (
            <div key={key as string}>
              ...
              <div className="relative">
                {prefix && <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">{format(0).replace(/[0-9,.]/g, "").replace(/[+-]/g, "")}</span>}
                <input type="number" step="any" value={form[key] as number} onChange={e => set(key, e.target.value)}
        ...
        <div className="px-5 pt-2">
          <button onClick={async () => { setSaving(true); await onSave(form); setSaving(false); onClose(); }} disabled={saving} className="btn-primary">
...
export default function Goals() {
  ...
  const { goals, loading, error: goalsError, saveGoals } = useGoals();
  ...
  const err = tradesError || goalsError;
    ...
    <>
      <div className="flex flex-col">
        <header className="page-header px-5 py-3.5 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2"><Target size={17} className="text-primary" /><h1 className="text-lg font-bold font-serif text-foreground">Goals & Streaks</h1></div>
            <p className="text-[11px] text-muted-foreground mt-0.5">Track your targets and consistency</p>
          </div>
          <button onClick={() => setEditing(true)} className="flex items-center gap-1.5 bg-primary/15 border border-primary/25 text-primary text-xs font-bold px-3 py-2 rounded-xl hover:bg-primary/25 active:scale-90 transition-all tap-small">
            <Edit3 size={11} /> Edit
          </button>
        </header>
        ...
        <div className="px-4 pt-4 pb-6 flex flex-col gap-4">
          {err && (
            <div className="flex items-start gap-3 bg-red-500/10 border border-red-500/25 rounded-2xl p-4">
              ...
              <div><p className="text-sm font-semibold text-red-300">Firebase error</p><p className="text-[11px] text-red-400/80 mt-0.5">{err}</p></div>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 stagger">
            {[
              ...
              <div key={label} className="glass-card rounded-2xl p-3.5 border border-white/[0.06] flex flex-col items-center text-center">
                <div className="p-2 rounded-xl mb-1.5" style={{ background: `${color}20` }}><Icon size={16} style={{ color }} /></div>
          ...
          <div className="flex flex-col gap-3 stagger">
            <GoalCard label="Weekly P&L"  current={Math.max(0,stats.weekPnl)}  target={goals.weeklyPnl}     icon={Zap}      color="#f6c948" format={v => format(v)} />
            <GoalCard label="Monthly P&L" current={Math.max(0,stats.monthPnl)} target={goals.monthlyPnl}    icon={TrendingUp} color="#10b981" format={v => format(v)} />
          ...
          <div className="glass-card rounded-3xl p-5 border border-white/[0.06]">
            ...
            <div className="flex flex-col divide-y divide-border/30">
              {[
                { label: "Week P&L",      ok: stats.weekPnl >= 0, display: format(stats.weekPnl), color: getPnlColor(stats.weekPnl) },
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2.5 stagger">
              {achievements.map(({ label, desc, earned, icon: Icon }) => (
                <div key={label} className={cn("glass-card rounded-2xl p-4 border transition-all card-hover", earned ? "border-yellow-400/25 bg-yellow-400/5" : "border-white/[0.04] opacity-50")}>
                  ...
                  {earned && <div className="flex items-center gap-1 mt-2"><Trophy size={9} className="text-yellow-400" /><span className="text-[9px] font-bold text-yellow-400">Earned</span></div>}
                </div>
              ))}
            </div>
          </div>
      ...
      {editing && <EditSheet goals={goals} onSave={saveGoals} onClose={() => setEditing(false)} format={format} />}
    </>
  );
}
